...
async function getWindowInfo(r) {
  ...
  // Removed date/time logic
  try {
    if (hasRedis(r)) {
      const sn = await r.get("showName").catch(() => "");
      if (sn) showName = sn;
    } else if (isLocal) {
  ...
  // Use sanitized showName as the permanent window key
  const windowKey = showName.replace(/\s+/g, "_").toLowerCase();
  return { showName, windowKey, ttlSeconds: 60 * 60 * 24 * 30 }; // Default 30 day TTL
}
...
export default async function handler(req, res) {
  ...
  /* ───── CONFIG (UPDATED: Removed Date Logic) ───── */
  if (action === "config") {
    if (req.method === "GET") {
      noCache(res);
      try {
        if (isLocal) {
          try {
            ...
            return res
              .status(200)
              .json({ showName, version });
          } catch {}
        }
        return await withRedis(async (r) => {
          let [showName, version] =
            await Promise.race([
              Promise.all([
  ...
  if (
    ...
    // Disabled auto-pick as requested
    return res.status(200).json({ ok: false, reason: "auto_pick_disabled" });
  }
  ...
  // ────────────────────────────────────────────────────────────
  // POLL: TOGGLE CLOSE/OPEN
  // ────────────────────────────────────────────────────────────
  if (action === "poll-toggle" && req.method === "POST") {
    if (!isAdmin(req)) return res.status(403).json({ error: "Forbidden" });
    ...
    try {
      return await withRedis(async (r) => {
        if (closed) {
          await r.set(`poll:closed:${pollId}`, "true");
          await r.set(`poll:closed_msg:${pollId}`, message || "Poll closed.");
        } else {
          await r.del(`poll:closed:${pollId}`);
        }
        return res.json({ success: true, closed: !!closed });
      }, 2500);
    } catch {
      return res.status(503).json({ error: "Redis not ready" });
    }
  ...
  if (action === "poll" && req.method === "GET") {
    try {
      return await withRedis(async (r) => {
        ...
        const [raw, ver, isClosed, closedMsg] = await Promise.all([
           r.get(`poll:songs:${pollId}`),
           r.get(`poll:version:${pollId}`).catch(() => "1"),
           r.get(`poll:closed:${pollId}`),
           r.get(`poll:closed_msg:${pollId}`)
        ]);
      ...
      return res.status(200).json({ pollId: "top10", songs: [], version: 1, closed: false });
    }
  }
  ...
  if (action === "poll-vote" && req.method === "POST") {
    try {
      return await withRedis(async (r) => {
        ...
        // --- NEW: Check if closed ---
        const isClosed = await r.get(`poll:closed:${pollId}`);
        if (isClosed === "true") {
           return res.status(403).json({ error: "Poll is currently closed." });
        }
        // ----------------------------
  ...
  if (action === "env-dump" && req.method === "GET") {
    const mask = (t) =>
